...
def generate_launch_description():
    ...
    launch_gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_dir, 'launch', '21_gazebo_world.launch.py')
        ),
    ...
    launch_spawn = TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_dir, 'launch', '22_spawn_robots.launch.py')
            )
        )]
    )
    ...
    launch_nav = TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_dir, 'launch', '23_navigation.launch.py')
            ),
