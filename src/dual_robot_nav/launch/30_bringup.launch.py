"""
Launch File: 30_bringup.launch.py
Purpose   : Single-command launcher that orchestrates all phases in order.
            Layout 4 Gazebo -> Spawn Robots -> Navigation -> Mission Controller

Usage:
    ros2 launch dual_robot_nav 30_bringup.launch.py
    ros2 launch dual_robot_nav 30_bringup.launch.py gui:=false  # headless
"""
...
def generate_launch_description():
    ...
    launch_gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_dir, 'launch', '31_gazebo_world.launch.py')
        ),
    ...
    launch_spawn = TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_dir, 'launch', '32_spawn_robots.launch.py')
            )
        )]
    )
    ...
    launch_nav = TimerAction(
        ...
        actions=[IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(pkg_dir, 'launch', '33_navigation.launch.py')
            ),
            launch_arguments={'rviz': LaunchConfiguration('rviz')}.items()
        )]
    )

    # ── Phase 4: Layout 4 Mission Controller C++ node (t=70s, after both Nav2 stacks are up)
    mission_controller = TimerAction(
        period=70.0,
        actions=[Node(
            package='dual_robot_nav',
            executable='4_mission_controller',
